...
4. **Invoke `superpowers:brainstorming`** with this framing:
   > "Here's a draft spec generated by /spec-issue for issue
   > #$ARGUMENTS. The committed spec is the file in `docs/superpowers/specs/`.
   > The discovery report (questions, options, recommendation,
   > assumptions, files I read) is in the PR body. Walk me
